...
# Compile Tailwind v4 → site/static/css/main.css. The offline Nix build runs
# only `hugo`, so it ships whatever main.css is staged — recompile + git add
# after editing src/input.css or any layout utility class.
assets:
    cd site && npx @tailwindcss/cli -i src/input.css -o static/css/main.css --minify
